...
/**class OrdinaryCollision : class for ordinary collision
*@author Mathieu GRONDIN
*@version 1.0
*/
public class OrdinaryCollision extends Collision{
	
	/**OrdinaryCollision class constructor
	*/
	public OrdinaryCollision(){
		this.rubber = false;
		this.ordinary = true;
	}
	...
	*/
	public void update(Player player,Coordinate coordJoueur,double dT,Tile tile[][],Item item[][],int lengthTile,int widthTile,int mapHeight){
		...
		//System.out.println("KR : " + player.getKR());
...
//		player.setCoordJoueurY(newCoordJoueur.getY());
	...
	//		player.setCoordJoueurY(player.getStartCoord().getY());
			coordJoueur.setX(player.getStartCoord().getX());
			coordJoueur.setY(player.getStartCoord().getY());
